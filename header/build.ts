...
/** 
 * @description 因为需要将ts编译成js，执行js再生产app_header.js文件，所以需要将编译的js删除
 * @description 因为 用于vue开发 tsconfig 选项设置"module": "esnext"， 所以无法直接ts-node 使用 esm 语法的文件，只能编译后再用node执行
 * 
 */
function deleteJsFile() {
    fs.unlinkSync(`${__dirname}/index.js`);
    fs.unlinkSync(`${__dirname}/build.js`);
    fs.unlinkSync(`${__dirname}/UserScript.js`);
}
